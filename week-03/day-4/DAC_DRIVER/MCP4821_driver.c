...
void MCP4821_Init() {
	...
	SPI_MOSI_DDR |= 1 << SPI_MOSI_DDR_POS; //set PINB3 as output - master out slave in channel
	SPI_SCK_DDR |= 1 << SPI_SCK_DDR_POS; //set PINB5 as output - master clock out
	SPI_SS_DDR |= 1 << SPI_SS_DDR_POS; //set PINB2 as output - slave select
