...
int main(void)
{
	...
	while (1) {
		...
		PORTB |= 1 << PORTB5;
		_delay_ms(50);	// Here the CPU will wait for 500ms

		// Now turn off the LED by writing the bit "0" to PORTB 6th bit (PORTB5)
		// The PORTB |= 0 << PORTB5 will not work. Do you know why?
		// PORTB |= 0 << PORTB5 will not work because with or ( | ), if either values are 1 (true), the result will be 1.

		//PORTB ^= 1 << PORTB5;
		PORTB &= ~(1 << PORTB5);

		_delay_ms(160);	// Here the CPU will wait for 500ms
	}
}
